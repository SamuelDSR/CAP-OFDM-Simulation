...
%       target      : channel gap or tagert BER
%       gainToNoise : channel gain of sub channel versus Noise 
%       nSubcar     : total Number of sub channel
%       maxOrder    : max constellation size
%       totalPower  : total Avaiable Power

function [bitAlloc, powerAlloc] = campello_algo(bitAlloc,target,gainToNoise,nSubcar,totalPower,maxOrder)
...
for i = 1:nSubcar
    for j = 2:maxOrder+1
        ...
        if rem(j,2) == 1 % if the bit/symbol is even, Square QAM
            phim = 1.5/(2^(j-1)-1);
            powerTable(j,i) = -log(5*target)/(phim*gainToNoise(i));
        else
            phim = 6/(5*2^(j-1)-1);
            powerTable(j,i) = -log(5*target)/(phim*gainToNoise(i));
        end          
...
while minIncreasePower < maxDecreasePower && minIndice ~= maxIndice
    ...
    if minIndice == maxIndice
        disp('oops, local optimum');
    end
end

%% current total Power allocation table
currentPower  = 0;
for i=1:nSubcar
...
%Note that total average power is "total Power", so the sum of power for nSubcar is nSubcar*totalPower
